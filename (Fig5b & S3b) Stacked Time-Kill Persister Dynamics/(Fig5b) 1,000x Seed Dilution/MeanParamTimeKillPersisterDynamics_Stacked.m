...
if drug == "Amp"
    ...
    k_1 = 8.374;
    k_2 = 0.816;
    init_pers_perc_1000x = 10^-1.512;
elseif drug == "Cip"
    f_1 = 10^-3.691;
    k_1 = 15.185;
    k_2 = 0.258;
    init_pers_perc_1000x = 10^-2.815;
elseif drug == "Kan"
    f_1 = 10^-6.052;
    k_1 = 22.896;
    k_2 = 0.629;
    init_pers_perc_1000x = 10^-5.379;
end
...
P_pe_0 = N_0 * (init_pers_perc_1000x/100);
...
%% Simulation - For extracting data at specific timepoints (Fig 5c)
...
% 
% N_at_t_eval = sum(y_ode_at_t_eval);
% 
% percent_S_at_t_eval = 100*y_ode_at_t_eval(1)/N_at_t_eval;
% percent_Ppe_at_t_eval = 100*y_ode_at_t_eval(2)/N_at_t_eval;
% percent_Pdi_at_t_eval = 100*y_ode_at_t_eval(3)/N_at_t_eval;
% 
% percents_of_interest_at_t_eval = [percent_S_at_t_eval, percent_Ppe_at_t_eval, percent_Pdi_at_t_eval];

%% Simulation - For making stacked plots of persister formation dynamics (Fig 5b)
...
ylim([0,100]);
yticks(0:25:100);
legend('Drug-Induced Persisters','Pre-existing Persisters','Susceptible Cells','Location','northeast','orientation','vertical');
grid on;
...
% Save plot
fig1_plot_name = strcat(drug,"_MeanParamTimeKillPersisterDynamics_Stacked.png");
