...
% Phase 1: Stationary Phase - Asssume that a fraction of stationary phase 
% cells are persisters (Type I persistence).

% Action 1: Dilution of stationary phase cells into fresh media at a range
% of dilution factors.
...
% Model Parameters
stat_cells_tot = 2.5*10^9; % Total CFU/ml in stationary phase, 2.5 to 4 times 10^9 in our batch culture experiments
stat_persister_frac = 0.5/100; % Fraction of stationary phase cells that are persisters, assume 0.5%

% Results
S_stat = stat_cells_tot*(1-stat_persister_frac); % Susceptible cells in stationary phase culture
P_1_stat = stat_cells_tot*(stat_persister_frac); % Type I persister cells in stationary phase culture
P_2_stat = 0; % Assume that there are no Type II persister cells in the stationary phase culture
P_tot_stat = P_1_stat + P_2_stat;
N_tot_stat = S_stat + P_tot_stat;

%% Dilution into fresh media

% Model parameters
dil_facts = [100; 1000; 10000]; % dilution factors
...
for i = 1:n_dilution_factors
    results_storage{i} = [0, S_stat/dil_facts(i), P_1_stat/dil_facts(i), P_2_stat/dil_facts(i), P_tot_stat/dil_facts(i), N_tot_stat/dil_facts(i)];
end

%% Lag phase and exponential phase (Balaban 2004 WT model)

% Balaban WT model parameters
a_2 = 100*1.2*10^-6*(1/60); % Per cell per hour times (1h / 60min), Given in Balaban Fig 2
...
% time-kill assay.
t_treatment = [90, 180, 270]; % 100x, 1000x, 10000x
...
% Our model parameters (from SDTK fitting of Amp_WT_Rep1 data)
k_1 = 7.378*(1/60); % deaths per cell per hour times (1h / 60min)
k_2 = 0.770*(1/60); % deaths per cell per hour times (1h / 60min)
f_1 = 0*0.00138*(1/60); % drug induction of persistence per cell per hour times (1h / 60min)
...
expdata = readmatrix("SDTK_Amp_WT_Rep1_022625_Data.txt");
...
for i = 1:n_dilution_factors
    ...
    semilogy(t_plot_h,log10_P_2_plot,'Color',[141,8,194]/255,"LineWidth",2);
...
legend('S','P_1','P_2','P_{tot}','N');

title("a_2 = 100x Reported Value   &   f_1 = 0");
...
% Save figure
saveas(f1,"SFig1g_100a2_0f1.png");
